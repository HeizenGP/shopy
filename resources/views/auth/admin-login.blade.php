...
<div class="rounded-[30px] border border-white/10 p-6 shadow-2xl backdrop-blur-xl sm:p-8 space-y-6" style="background-color: var(--color-admin-login-panel); border-color: rgba(255, 255, 255, 0.1);">
        ...
        <form action="{{ route('admin.login') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                       style="background-color: var(--color-admin-login-bg); color: var(--color-admin-login-text); border-color: rgba(0,0,0,0.1);"
        ...
        <div class="p-4 rounded-2xl border text-left text-xs" style="background-color: var(--color-admin-login-bg); border-color: rgba(0,0,0,0.05); color: var(--color-admin-login-muted);">

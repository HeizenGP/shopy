@php
    $adminUser = auth()->user();
    $adminHomeRoute = \App\Access\Presentation\Support\AdminNavigation::firstRouteNameFor($adminUser);
    $catalogOpen = request()->routeIs('admin.catalog.*');
    $accessOpen = request()->routeIs('admin.access.*');
    $canViewDashboard = $adminUser?->hasPermission('dashboard.view');
    $canManageUsers = $adminUser?->hasPermission('access.manage_users');
    $canManageRoles = $adminUser?->hasPermission('access.manage_roles');
    $canManagePermissions = $adminUser?->hasPermission('access.manage_permissions');
    $canManageProducts = $adminUser?->hasPermission('catalog.manage_products');
    $canManageCategories = $adminUser?->hasPermission('catalog.manage_categories');
    $canManageBrands = $adminUser?->hasPermission('catalog.manage_brands');
    $canManageVariants = $adminUser?->hasPermission('catalog.manage_variants');
    $canViewAccessMenu = $canManageUsers || $canManageRoles || $canManagePermissions;
    $canViewCatalogMenu = $canManageProducts || $canManageCategories || $canManageBrands || $canManageVariants;
@endphp

<aside class="admin-sidebar">
    <a href="{{ route($adminHomeRoute ?? 'admin.no-permissions') }}" class="sidebar-logo">
